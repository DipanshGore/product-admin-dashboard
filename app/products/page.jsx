...
export default function DashboardPage() {
  ...
  // Read URL params with fallbacks for invalid values (?page=abc -> 1)
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // If the request was intentionally cancelled by a URL redirect, fail silently
      if (axios.isCancel(err) || err.name === 'CanceledError' || err.name === 'AbortError') {
        return;
      }
      setError(err.message || 'Failed to load products.');
    } finally {
      // Only clear the loading state if this specific request wasn't aborted by a newer one
 ...
 const handleFormSubmit = async (e) => {
    ...
    try {
      if (editingProduct) {
        // 1. Try to update via API, but catch and ignore 404 errors for mock items
        try {
        ...
        // 2. Force local state update regardless of backend persistence
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsDeleting(true);
    try {
      // 1. Try to delete via API, but catch and ignore 404 errors for mock items
      try {
      ...
      // 2. Force removal from local state
